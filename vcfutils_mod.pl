...
# Author: lh3
# modified by Aarti Venkat and Wynn Meyer
...
sub main {
  ...
  my %func = (subsam=>\&subsam, listsam=>\&listsam, fillac=>\&fillac, qstats=>\&qstats, varFilter=>\&varFilter,
			  ...
			  gapstats=>\&gapstats, splitchr=>\&splitchr, vcf2fq=>\&vcf2fq, vcf2fqnonref=>\&vcf2fqnonref);
...
sub varFilter {
  my %opts = (d=>2, D=>10000000, a=>1, W=>0, Q=>10, w=>0, p=>undef, 1=>1e-4, 2=>1e-100, 3=>0, 4=>1e-4, G=>0, S=>1000);
...
#### aatu ######
	...
	### aatu ######
		#foreach my $elem(@staging){
		#	print "staging\t\t@{$elem}\n";
		#	print "max dist\t$max_dist\tstaging element\t$staging[0][2]\t$staging[0][3]\t$staging[0][4]\tt values 0 and 1\t$t[0]\t$t[1]\n";
		#}
	### aatu ####

# Still on the same chromosome and the first element's window still affects this position?
	  
	####### aatu commented this line below ####
...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  while (<>) {
	...
	### aatu comment below ### this code is parsing based on FQ field, so comment the line below ###
	...
	#### aatu insert  below ######  
	  if($t[4]=~/\./){
		$b = $ref ;
		#$q = ( $t[5] ne ".")? $t[5]: 100; # since FQ is neg at homs
		
		## NOTE: FQ is negative at homs, but a negative value creates problems
                ## to display the quality values, due to some mess-up in perl unicode
		## encoding; Just arbitarily displaying qual values here

		if($t[5] ne "." && $t[5] <= 126){
			$q = $t[5];
		}
		elsif($t[5] ne "." && $t[5] > 126){
			$q = 126;
		}
		else{
			$q = 1; # when there is no qual-score, assume its very low qual/ lowest read support
		}
		#print "$b\t$q\n";    
	}
...
sub vcf2fqnonref {
  ...
  getopts('d:D:Q:l:', \%opts);
  die(qq/
Usage:   vcfutils.pl vcf2fqnonref [options] <all-site.vcf>
...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  while (<>) {
        ...
        die("[vcf2fqnonref] unsorted input\n") if ($t[1] - $last_pos < 0);
        if ($t[1] - $last_pos > 1) {
          $seq .= 'n' x ($t[1] - $last_pos - 1);
          $qual .= '!' x ($t[1] - $last_pos - 1);
        }
        ...
        #### aatu insert  below ######
        #### further modified by wm #####
          if($t[4]=~/\./){

        #### This should default to the reference only if there are reads at this site. ###
        #### Otherwise, should be "N." ####
        #### If there are reads, there should be a quality score ($t[5]). ####
        #### If using output from an earlier version, mark anything with QUAL = 1 as N.####               
                ...
                if($t[5] ne "." && $t[5] <= 126){
                        ...
                        $b = "N" #new addition: if there are any reads for the non-reference, it will have a quality score
                        $q = 1; # when there is no qual-score, assume it's very low qual/lowest read support
                }
                #print "$b\t$q\n";
        }
        ...
        ### wm modified ###############
                ...
                if($t[9]=~/0\/1/){ #Sample is heterozygous reference/non-reference.
                    #print "$ref\t$alt\n";
                    $b = $het{"$ref$alt"};
                    #print "$b\n";
                    $b ||= 'N';
                    #print "$b\n";
                    my @data = split(':',$t[9]);
                    my $GQ = $data[3]; #the 4th element in the colon-separated 9th column
                    $q = -10*log(1-(1-10**(-$t[5]/10))*(1-10**(-$GQ/10)))/log(10); #modify this to: tophred(1-(1-fromphred(QUAL))*(1-fromphred(GQ)))
                    #Comment the following line out in the final version:
                    #print "$b\t$q\n";
                }
                elsif($t[9]=~/1\/1/){ #Sample is homozygous non-reference.
                    #print "$ref\t$alt\n";
                    $b = $alt ;
                    #print "$b\n";
                    $b ||= 'N';
               ...
               elsif ($t[9]=~/1\/2/){ #Sample is heterozygous with both alleles non-reference.
                   my $altgeno = $t[4];
                   $altgeno =~ s/,//g;
                   $b = $het{"$altgeno"};
                   $b ||= 'N';
                   ...
                   print "Genotype and quality not found for line @t\n";
                   exit 1;
               }
        }
          $b = lc($b);
...
Command: subsam       get a subset of samples
         ...
         ### wm modified##
         vcf2fq       GATK all-sites VCF from mapping to a self-reference -> fastq (**)
         vcf2fqnonref GATK all-sites VCF from mapping to different reference -> fastq (**)
         ## wm edits done
